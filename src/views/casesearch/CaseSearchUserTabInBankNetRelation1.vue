<template>
  ...
  <div>
    <div style="margin: 10px;font-size: 1rem">网申进件</div>
    <el-table
      ref="table"
      v-loading="loading"
      :data="tableData"
...
import {queryCaseUserInBankRelationOnline} from "@/api/cases";
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      ...
      // @ts-ignore
      const {data: {items, total}} = await queryCaseUserInBankRelationOnline(condition)
